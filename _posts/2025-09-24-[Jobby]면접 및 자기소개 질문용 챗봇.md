...
먼저 Dialogflow가 묶일 Google Cloud 프로젝트부터 하나 만든다.[web:36]
...
Dialogflow API를 서버에서 호출하려면 **서비스 계정**이 필요하다.[web:36]
...
이제 방금 만든 서비스 계정에 대해 **키(JSON 파일)**를 하나 발급한다.[web:36]
...
이제 Dialogflow 쪽에서 에이전트를 생성해서, 방금 만든 GCP 프로젝트와 묶는다.[web:38]
...
이제 서버 쪽으로 돌아와서, 아까 받은 JSON 파일을 열고 `dev.js`를 채워 넣는다.[web:37][web:40]
